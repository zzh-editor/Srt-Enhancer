...
ASR 经常将专业术语误识别为发音相似的常见词。使用 `references/correction-table.md` 术语对照表进行修正。
...
2. **Check User Overrides** (highest priority): Apply `terminology_overrides` with `mode=auto` (exact → casefold → normalized)
3. **Check Static Correction Table**: Apply `correction-table.md` entries with `mode=auto`
4. **Web Verify**: Only for terms not matched by any table; use domain's `search_context` for focused queries
5. **Validate Context**: Ensure correction makes sense in subtitle context
6. **Flag Uncertain**: Low-confidence corrections (no table match, no web source) → mark ❗ for user
...
2. Static references (correction-table.md)
